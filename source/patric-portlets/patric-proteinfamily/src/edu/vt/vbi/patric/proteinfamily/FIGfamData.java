...
public class FIGfamData {
	...
	/*
	 * private String[] getFigfamPair(ResourceRequest req) { String[] result = null; String genomeIds = req.getParameter("genomeIds"); String
	 * figfamIds = req.getParameter("figfamIds"); if ((genomeIds == null) || ((genomeIds.trim()).length() == 0)) { if (figfamIds != null) {
	 * PortletSession sess = req.getPortletSession(true); ResultType key = (ResultType) (sess.getAttribute("key" + figfamIds,
	 * PortletSession.APPLICATION_SCOPE)); if (key != null) { genomeIds = key.get("genomeIds"); figfamIds = key.get("figfamIds"); } } } if ((figfamIds
	 * != null) && (0 < figfamIds.length()) && (genomeIds != null) && (0 < genomeIds.length())) { result = new String[PAIR_ROOM]; result[GENOMES_AT] =
	 * genomeIds; result[FIGFAMS_AT] = figfamIds; } return result; }
	 * 
	 * private String getFigfamNameRestrict(String fieldName, String figfamIds) { String[] figList = figfamIds.split(","); StringBuilder build = new
	 * StringBuilder(0x200); restrictByList(fieldName, figList, build); return (build.toString()); }
	 */
	...
	/*
	 * private void addPopupHeader(PrintWriter writer) { writer.write( "<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\"\n" +
	 ...
	 * "<div id=\"masthead\">" + "<a href=\"/\">PATRIC <span class=\"sub\">" + "Pathosystems Resouce Integration Center</span></a></div>" + "</div>\n"
	 * + "<div id=\"toppage\">\n" + "<div class=\"content\">\n"); }
	 * 
	...
	/*
	 * private int[] splitOutGenomeIds(String idText) { int[] result = null; if (idText != null) { String[] textIds = idText.split(","); result = new
	 * int[textIds.length]; for (int i = 0; i < result.length; i++) { result[i] = Integer.parseInt(textIds[i]); } } return result; }
	 */
	...
	/*
	 * private void restrictByList(String restricted, String[] textValues, StringBuilder toBuild) { int count = textValues.length; if (0 < count) { if
	 * (count == 1) { toBuild.append(restricted + " = '" + textValues[0] + "'"); } else if (count <= IN_MAX) { toBuild.append(restricted + " in ('" +
	 * textValues[0] + "'"); for (int i = 1; i < count; i++) { toBuild.append(", '" + textValues[i] + "'"); } toBuild.append(")"); } else {
	 * toBuild.append("((" + restricted + " in ('" + textValues[0] + "'"); int at = 1; int nextBreak = IN_MAX; while (at < count) { String value =
	 * textValues[at]; if (at <= nextBreak) { toBuild.append(", '" + value + "'"); } else { toBuild.append(")) or (" + restricted + " in ('" + value +
	...
	public void getGroupStats(ResourceRequest req, PrintWriter writer) throws IOException {
			...
			for (Map.Entry<String, List<PivotField>> pivot : pivots) {
				...
				if (pivotEntries != null) {
					for (PivotField pivotEntry : pivotEntries) {
						...
						/*
						 * for(int i=0; i<genomeIds.length; i++){ genomeIdsStr[i] = "00"; }
						 */
		...
		/*
		 *  *:*&fq=gid:(252168 OR 25663 OR 244652 OR 223303) AND figfam_id:(FIG00002864 OR
		 * FIG00450233)&group=true&group.field=figfam_id&fl=figfam_id,figfam_product
		 */
			...
			if (iter.hasNext()) {
				...
				while (iter.hasNext()) {
					...
					if (figfamName.equals(nextName)) {
						...
						pastStart = true;
						setFigFamStat(figfamName, proteinCount, idFinder.length, members, minAminos, maxAminos, description, writer);
						figfamName = nextName;
	...
	/*
	 * private static void setFigFamStat(String figfamName, int proteinCount, int genomeCount, int[] members, int minAminos, int maxAminos, int
	 * median, double std, String description, PrintWriter writer) { writer.write(figfamName + "\t"); int speciesCount = 0; for (int i = 0; i <
	 * genomeCount; i++) { int value = members[i]; if (0 < value) { ++speciesCount; value = Math.min(value, 255); } writer.write(hexDigits[value]); }
	 * 
	 * writer.write("\t" + proteinCount + "\t" + speciesCount + "\t" + minAminos + "\t" + maxAminos + "\t" + median + "\t" + std + "\t" +
	 * description);
	 * 
	 * }
	 */

	private static void setFigFamStat(String figfamName, int proteinCount, int genomeCount, int[] members, int minAminos, int maxAminos,
			String description, BufferedWriter writer) throws IOException {
		...
		writer.write("\t" + proteinCount + "\t" + speciesCount + "\t" + minAminos + "\t" + maxAminos + "\t" + description);
	}
	...
	/*
	 * private int getSqlInt(StringBuilder query) { Session session = factory.getCurrentSession(); session.beginTransaction(); SQLQuery q =
	 ...
	 * 
	 * private String getSqlString(StringBuilder query) { Session session = factory.getCurrentSession(); session.beginTransaction(); SQLQuery q =
	 * session.createSQLQuery(query.toString());
	 * 
	 * Object results = q.uniqueResult(); session.getTransaction().commit();
	 * 
	...
	/*
	 * private class FigfamFeature implements Comparable<Object> { int featureId;
	 * 
	 * String figfamName;
	 * 
	 ...
	 * String product = null;
	 * 
	 * FigfamFeature(Object[] selects) { featureId = getObjectInt(selects[0]); figfamName = (String) selects[1]; figfamProduct = (String) selects[2];
	 * }
	 * 
	 * int addDetails(Object[] selects, ArrayList<FigfamFeature> keepers) { int result = (featureId - getObjectInt(selects[0])); if (result == 0) {
	 * genomeName = "" + selects[1]; genomeId = getObjectInt(selects[2]); accession = "" + selects[3]; sourceId = "" + selects[4]; name = "" +
	 * selects[5]; startMax = "" + selects[6]; endMin = "" + selects[7]; span = "" + selects[8]; isReversed = "" + selects[9]; aaLength =
	 * getObjectInt(selects[10]); gene = "" + selects[11]; product = "" + selects[12]; keepers.add(this); } return result; }
	 * 
	 * void stampHeader(int count, PrintWriter writer) { writer.write(count + "\t" + figfamName + "\t" + figfamProduct); }
	 * 
	 * void stampHeader(int count, ArrayList<String> collect) { collect.add("" + count); collect.add(figfamName); collect.add(figfamProduct); }
	 * 
	 * void stampDetails(ArrayList<String> collect) { collect.add(genomeName); collect.add(accession); collect.add(sourceId); collect.add(name);
	 * collect.add(startMax); collect.add(endMin); collect.add(span); collect.add(isReversed); collect.add("" + aaLength); collect.add(gene);
	 * collect.add(product); }
	 * 
	 * void stampDetails(PrintWriter writer) { writer.write("\t" + genomeName); writer.write("\t" + genomeId); writer.write("\t" + accession);
	 * writer.write("\t" + featureId); writer.write("\t" + sourceId); writer.write("\t" + name); writer.write("\t" + startMax); writer.write("\t" +
	 * endMin); writer.write("\t" + span); writer.write("\t" + isReversed); writer.write("\t" + aaLength); writer.write("\t" + gene);
	 * writer.write("\t" + product); }
	 * 
	 * boolean inSameGroup(FigfamFeature o) { return (this.figfamName).equals(o.figfamName); }
	 * 
	 * public int compareTo(Object other) { FigfamFeature o = (FigfamFeature) other; int result = (this.figfamName).compareTo(o.figfamName); if
	 * (result == 0) { result = (this.genomeName).compareTo(o.genomeName); } return result; } }
	 */
	/*
	 ...
	 * 
	 * SolrFamily(JSONObject selects) { this.figfam_id = selects.get("figfam_id").toString(); this.figfam_product =
	 * selects.get("figfam_product").toString(); this.na_feature_id = selects.get("na_feature_id").toString(); this.gid =
	 * selects.get("gid").toString(); this.min_length = Integer.parseInt(selects.get("aa_length").toString()); this.max_length =
	 * Integer.parseInt(selects.get("aa_length").toString()); this.features = 1;
	 * this.lengths.add(Integer.parseInt(selects.get("aa_length").toString())); }
	 * 
	 * public int getTheMedian() {
	 * 
	 * Collections.sort(this.lengths); int data = this.lengths.get(this.lengths.size() / 2); return data;
	 * 
	 * }
	 * 
	 * public double getTheStd(ArrayList<Integer> list) {
	 * 
	 * double max = list.get(0).doubleValue(); double mean = 0.0; double brackets = 0.0;
	 * 
	 * for (int i = 0; i < list.size(); i++) { if (list.get(i).doubleValue() > max) max = list.get(i).doubleValue(); mean +=
	 * list.get(i).doubleValue(); } mean = mean / list.size();
	 * 
	 * for (int j = 0; j < list.size(); j++) { brackets += (Math.pow((list.get(j).doubleValue() - mean), 2) / (list.size() - 1)); } brackets =
	 * Math.sqrt(brackets); // System.out.print(brackets); if (brackets > 0) { DecimalFormat twoDForm = new DecimalFormat("#.#"); return
	 * Double.valueOf(twoDForm.format(brackets)); } else { return 0.0; } } }
	 */

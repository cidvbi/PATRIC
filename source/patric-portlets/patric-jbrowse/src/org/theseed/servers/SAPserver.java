...
public class SAPserver {
	 ...
	 * 
	 * You will not use the methods in this section very often. Some are used by the server framework for maintenance
	 * and control purposes ("methods"), while others ("query" and "get") provide access to data in the database in case
	 * you need data not available from one of the standard methods.
	 */
	...
	/**
	 * Return the assertions for all genes in the database that match the identified gene. The gene can be specified by
	 * any prefixed gene identifier (e.g. uni|AYQ44, gi|85841784, or fig|360108.3.peg.1041).
	 * 
	 * @param ids an array of IDs
	 * 
	 * @return Returns a reference to a hash that maps each incoming ID to a list of 4-tuples. Each 4-tuple contains (0)
	 * an identifier that is for the same gene as the input identifier, (1) the asserted function of that identifier,
	 * (2) the source of the assertion, and (3) a flag that is TRUE if the assertion is by a human expert.
	 */
	...
	/**
	 * Return the assertions for all genes in the database that match the identified protein sequences. A protein
	 * sequence can be identified by a prefixed MD5 code or any prefixed gene identifier (e.g. uni|AYQ44, gi|85841784,
	 * or fig|360108.3.peg.1041).
	 * 
	 * @param ids Reference to a list of protein identifiers. Each identifier should be a prefixed gene identifier or
	 * the md5|-prefixed MD5 of a protein sequence.
	 * 
	 * @return Returns a hash mapping each incoming protein identifier to an ArrayList of 5-tuples, consisting of (0) an
	 * identifier that is sequence-equivalent to the input identifier, (1) the asserted function of that identifier, (2)
	 * the source of the assertion, (3) a flag that is TRUE if the assertion is by an expert, and (4) the name of the
	 * genome relevant to the identifer (if any).
	 */
	 ...
	 * @param type If specified, the type of feature desired (peg, rna, etc.). If omitted, all features will be
	 * returned.
	 * @param hypothetical If true only hypothetical genes will be returned If undefined or not specified, all genes
	 * will be returned.
	 ...
	 * 
	 * @param ids An array of of prefixed feature IDs (e.g. gi|17017961, NP_625335.1, fig|360108.3.peg.1041). The
	 * assertions associated with each particular identifier will be returned. In this case, there will be no processing
	 * for equivalent IDs. For that, you should use equiv_sequence_assertions or equiv_precise_assertions.
	 * 
	 * @return Returns a hash mapping every incoming ID to an ArrayList of 3-tuples, each consisting of (0) an asserted
	 * function, (1) the source of the assertion, and (2) a flag that is TRUE if the assertion was made by an expert.
	 */
	 ...
	 * @param source Database source of the IDs specified-- SEED for FIG IDs, GENE for standard gene identifiers, or
	 * LocusTag for locus tags. In addition, you may specify RefSeq, CMR, NCBI, Trembl, or UniProt for IDs from those
	 * databases. Use mixed to allow mixed ID types (though this may cause problems when the same ID has different
	 * meanings in different databases). Use prefixed to allow IDs with prefixing indicating the ID type (e.g.
	 * uni|P00934 for a UniProt ID, gi|135813 for an NCBI identifier, and so forth). The default is SEED.
	 * 
	 * @return Returns a reference to a hash mapping each feature ID to the feature's current functional assignment.
	 * Features that do not exist in the database will not be present in the results. For IDs that correspond to
	 * multiple features, only one functional assignment will be returned.
	 */
	 ...
	 * @return A hash that maps each specified role ID or functional assignment to a list of the FIG IDs of genes that
	 * have that role or assignment.
	 */
	...
	public HashMap<String, ArrayList<HashMap<String, Object>>> compared_regions(String focus, int count,
			String[] genomes, String[] pins, int extent) {
		server.reset();
	 ...
	 * 
	 * @param sequences Reference to a list of protein specifications. A protein specification can be a FASTA string, a
	 * 3-tuple consisting of (0) a label, (1) a comment, and (2) a protein sequence, OR a 2-tuple consisting of (0) a
	 * label and (1) a protein sequence. In other words, each specification can be a raw FASTA string, a parsed FASTA
	 * string, or a simple [id, sequence] pair. In every case, the protein sequence will be used to find identifiers and
	 * the label will be used to identify the results.
	 * 
	 * @return A hash mapping each incoming label to a list of identifiers from the database that name the protein or a
	 * feature that produces the protein.
	 */
	 ...
	 * uni|P00934 for a UniProt ID, gi|135813 for an NCBI identifier, and so forth). The default is SEED.
	 * 
	 * @param protein if set, the output FASTA sequences will be protein sequences; otherwise, they will be DNA
	 * sequences
	 * 
	 * @param fasta if set, the output sequences will be multi-line FASTA strings instead of sequences.
	 * 
	 * @param comments Allows the user to add a label or description to each FASTA formatted sequence. The HashMap has
	 * keys that are the ids, and the values are the desired labels. This parameter is only used when the -fasta option
	 * is specified.
	 * 
	 * @return a hash mapping the incoming IDs to sequence strings. IDs that are not found in the database will not
	 * appear in the hash.
	 */
	public HashMap<String, String> idsToSequences(String[] ids, String source, boolean protein, boolean fasta,
	 ...
	 * @param fasta if set, the output sequences will be multi-line FASTA strings instead of sequences
	 * 
	 * @return A hash that maps the incoming IDs to FASTA sequences for the specified DNA locations. The FASTA ID will
	 * be the ID specified in the incoming hash.
	 */
	...
	/**
	 * Return the DNA sequences for the upstream regions of the specified features. The nucleotides inside coding
	 * regions are displayed in upper case; others are displayed in lower case.
	 ...
	 * @param skipGene If TRUE, only the upstream region is included. Otherwise, the content of the feature is included
	 * in the output.
	 * @param fasta If TRUE, the output sequences will be multi-line FASTA strings instead of sequences.
	 * @param comments Allows the user to add a label or description to each FASTA formatted sequence. The values is a
	 * reference to a hash whose keys are the ids, and the values are the desired labels. This parameter is only used
	 * when the -fasta option is specified.
	...
	/**
	 * Return all identifiers for genes in the database that are protein-sequence-equivalent to the specified
	 * identifiers. In this case, the identifiers are assumed to be in their natural form (without prefixes). For each
	 * identifier, the identified protein sequences will be found and then for each protein sequence, all identifiers
	 * for that protein sequence or for genes that produce that protein sequence will be returned.
	 * 
	 * Optionally, you can ask for identifiers that are precisely equivalent, that is, that identify the same location
	 * on the same genome.
	 * 
	 ...
	 * @param boundaries If true, for any multi-location feature, a single location encompassing all the location
	 * segments will be returned instead of a list of all the segments. If the segments cross between contigs, then the
	 * behavior in this mode is undefined (something will come back, but it may not be what you're expecting). If this
	 * is false (or ommitted) the locations for each feature will be presented in a list.
	 * @return Returns a reference to a hash mapping each feature ID to a list of location strings representing the
	 * feature locations in sequence order
	 */
	 ...
	 * @param locations Reference to a list of location strings (e.g. 360108.3:NZ_AANK01000002_264528_264007 or
	 * 100226.1:NC_003888_3766170+612). A location string consists of a contig ID (which includes the genome ID), an
	 * underscore, a begin offset, and either an underscore followed by an end offset or a direction (+ or -) followed
	 * by a length.
	 * @param includeLocation If true, then instead of mapping each location to a list of IDs, the hash will map each
	 * location to a hash reference that maps the IDs to their locations.
	 * @return Returns a reference to a hash mapping each incoming location string to a list of the IDs for the features
	...
	/**
	 * Get a list of features for a list of genomes. Takes an array of genome ids and a feature type (peg, rna, pp,
	 * etc), and returns a hashMap with the keys being genomes and the values being ids of those features. If the type
	 * is ommitted all features will be returned.
	 * 
	 ...
	 * @param ids A list of identifiers. Each identifier can be a prefixed feature ID (e.g. fig|100226.1.peg.3361,
	 * uni|P0AC98) or a genome ID (83333.1, 360108.3).
	 * @param numbers (optional) If true, the genome ID number will be returned instead of the name. Note that this
	 * facility is only useful when the incoming identifiers are feature IDs, as genome IDs would be mapped to
	 * themselves
	 * @return Returns a reference to a hash mapping each incoming feature ID to the scientific name of its parent
	 * genome. If an ID refers to more than one real feature, only the first feature's genome is returned.
	 */
	...
	/**
	 * Return the representative genome for each specified incoming genome ID. Genomes with the same representative are
	 * considered closely related, while genomes with a different representative would be considered different enough
	 * that similarities between them have evolutionary significance.
	...
	/**
	 * Compute mappings for the genome sets (OTUs) in the database. This method will return a mapping from each genome
	 * to its genome set ID and from each genome set ID to a list of the genomes in the set. For the second mapping, the
	 * first genome in the set will be the representative.
	 * @return Returns a reference to a 2-tuple. The first element is a reference to a hash mapping genome IDs to genome
	 * set IDs; the second element is a reference to a hash mapping each genome set ID to a list of the genomes in the
	 * set. The first genome in each of these lists will be the set's representative.
	 */
	...
	public HashMap<String, ArrayList<String[]>> idsToSubsystems(String[] ids, boolean usable, String[] exclude,
			String source, String genome) {
		return this.idsToSubsystems(ids, usable, exclude, source, genome, false);
	}

	public HashMap<String, ArrayList<String[]>> idsToSubsystems(String[] ids, boolean usable, String[] exclude,
			String source, String genome, boolean subsOnly) {
		server.reset();

...
String experimental = request.getParameter("experimental");
...
if (key == null) {
	...
	keyword = key.get("keyword_search");
}
...
<form id="fTableForm_Feature" action="/patric-searches-and-tools/jsp/grid_download_handler.jsp" method="post">
...
<form id='<%=name%>_orthoToFile' action='<%=contextPath%>/jsp/GetMainTable.jsp' method="post">
	<input name='OrthoFileType' type='hidden' value='xls' />
	<input name='OrthoFileName' type='hidden' value='mainName' />
	<input name='data' type='hidden' value='' />
</form>

<form id='<%=name%>_detailsToFile'
	action='<%=contextPath%>/jsp/DetailsFromMain.jsp'
	method="post">
	<input name='detailsType' type='hidden' value='xls' />
	<input name='detailsName' type='hidden' />
	<input name='detailsGenomes' type='hidden' />
	<input name='detailsFigfams' type='hidden' />
	<input name='FAM_TYPE' type='hidden' />
	<input id = "portlet_type" name="portlet_type" type="hidden" value="figfam" />
</form>

<form id='<%=name%>_forDetails'
	action='FIGfamViewer/FigFamViewerWindow?action=1'
	target='_blank'
	method="post">
	<input name='figfam_names' type='hidden' />
	<input name='random_key' type='hidden' />
	<input name='key_word' type='hidden' value="<%=key.get("keyword") %>" />
	<input name='taxon_name' type='hidden' value="<%=key.get("genera") %>" />
	<input name='genome_ids' type='hidden' value="<%=key.get("genomeIds") %>" />
	<input name='FAM_TYPE' type='hidden' value="" />
</form>
...
Ext.onReady(function() {
	...
	FigFamSorterOnReady('<%=name%>', '<%=resourceURL%>',
						'<%=contextPath%>', '<%=cType%>', '<%=cId%>', '<%=keyword%>', '<%=experimental%>');
